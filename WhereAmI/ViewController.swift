...
class ViewController: UIViewController, CLLocationManagerDelegate {
    ...
    @IBOutlet var longituteLabel:UILabel!
    ...
    func locationManager(manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        if let newLocation = locations.last {
            let latitudeString = String(format: "%g\u{0080}", newLocation.coordinate.latitude)
            latitudeLabel.text = latitudeString

            let longitudeString = String(format: "%g\u{0080}", newLocation.coordinate.longitude)
            longituteLabel.text = longitudeString
